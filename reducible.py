...
def step_size(s):
    ...
    """
    original_hash = hash_word(s, STEP_SIZE_CONSTANT)
    steps_size = STEP_SIZE_CONSTANT - (original_hash % STEP_SIZE_CONSTANT)
    return steps_size
...
def insert_word(s, hash_table):
    ...
    start_index = hash_word(s, length)
...
def find_word(s, hash_table):
    ...
    """
    start_marker = hash_word(s, len(hash_table))
    pos = start_marker
    step = step_size(s)
...
def get_longest_words(string_list):
    ...
    for word in string_list:
        if not temp:
            temp.append(word)
            continue
        if len(word) > len(temp[0]):
            temp = []
            temp.append(word)
        elif len(word) == len(temp[0]):
            temp.append(word)
    return temp
...
def main():
    ...
    totl_len = len(word_list)
    # determine prime number N that is greater than twice
    # the length of the word_list
    prime_number_n = 2 * totl_len
    while not is_prime(prime_number_n):
        prime_number_n += 1
    # create an empty hash_list
    hash_list = []
    # populate the hash_list with N blank strings
    hash_list = [""] * prime_number_n
    # hash each word in word_list into hash_list
    # for collisions use double hashing
    for word in word_list:
        index = hash_word(word, prime_number_n)
        step = step_size(word)
        while hash_list[index] != "":
            index = (index + step) % prime_number_n
        hash_list[index] = word
